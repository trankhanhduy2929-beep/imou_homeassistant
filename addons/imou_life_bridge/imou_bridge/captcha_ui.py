"""Ingress pages for the visual CAPTCHA selected by Imou."""
...
<base href="{html.escape(base_path, quote=True)}"><title>Xác minh Imou Life</title>
...
<p>Hoàn tất CAPTCHA do Imou cung cấp để add-on tiếp tục đăng nhập bằng account/password.</p>"""


def _footer() -> str:
    return """<p class="hint">Trang chỉ gửi kết quả CAPTCHA về add-on qua Ingress. Account, password, verifyToken và app credential không được đưa vào trình duyệt.</p></main></body></html>"""


def _poll_script(generation: int) -> str:
    return """<script>
async function readStatus() {{ try {{ const response = await fetch("api/status", {{cache:"no-store"}}); const data = await response.json(); if (!data.captcha_required) {{ location.reload(); return; }} if (data.captcha_generation !== generation) location.reload(); }} catch (error) {{}} }}
...
def render_captcha_page(
    ...
    if challenge.is_image:
        return (
            _head(base_path)
            + _intro()
            + f"""<p><img id="captcha-image" src="api/captcha/image?g={generation}" alt="Ảnh CAPTCHA Imou"></p>
...
async function refreshCaptcha() {{ document.getElementById("refresh").disabled = true; status.textContent = "Đang lấy CAPTCHA mới…"; try {{ const response = await fetch("api/captcha/refresh", {{method:"POST"}}); const data = await response.json(); if (!response.ok || !data.ok) throw new Error(data.error || "Không đổi được CAPTCHA."); location.reload(); }} catch (error) {{ status.textContent = error.message; document.getElementById("refresh").disabled = false; }} }}
document.getElementById("refresh").addEventListener("click", refreshCaptcha);
document.getElementById("captcha-form").addEventListener("submit", async function(event) {{ event.preventDefault(); const button = document.getElementById("submit"); button.disabled = true; status.textContent = "Đang kiểm tra CAPTCHA…"; try {{ const response = await fetch("api/captcha", {{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{generation:generation,code:document.getElementById("captcha-code").value}})}}); const data = await response.json(); if (!response.ok || !data.ok) {{ if (data.generation !== generation) location.reload(); throw new Error(data.error || "CAPTCHA không hợp lệ."); }} status.textContent = "Xác minh thành công. Đang đăng nhập Imou…"; }} catch (error) {{ status.textContent = error.message; button.disabled = false; }} }});
</script>"""
